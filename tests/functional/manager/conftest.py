...
def token0_with_WETH9(
    ...
    _token0.approve(manager.address, 2**256 - 1, sender=sender)
...
def token1_with_WETH9(
    pool_with_WETH9, token_a, WETH9, sender, callee, manager, spot_reserve1, chain
):
    _token1 = WETH9 if pool_with_WETH9.token1() == WETH9.address else token_a
    _token1.approve(callee.address, 2**256 - 1, sender=sender)
    _token1.approve(manager.address, 2**256 - 1, sender=sender)
...
def pool_with_WETH9_initialized_with_liquidity(
    ...
    pool_with_WETH9.approve(callee.address, 2**256 - 1, sender=sender)
