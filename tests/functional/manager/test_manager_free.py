...
def test_manager_free__transfers_funds(
    ...
    assert token.balanceOf(alice.address) == balance_alice + margin_out
    assert (
...
def test_manager_free__pays_WETH9(
    ...
    )  # shouldn't change less gas fee
    assert WETH9.balance == balancee_WETH9  # shouldn't change
    assert alice.balance == balancee_alice  # shouldn't change
